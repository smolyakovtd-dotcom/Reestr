...
/**
 * index.php
 * Вся конфигурация (DB/SMTP/AUTH/session/csrf/types + smtp_send_text_mail) вынесена в config.php
 */
...
if (isset($_POST['print_mode']) && $_POST['print_mode'] === '1') {
    ...
    if ($canManageRegistry) {
        ...
        if (count($used) > 1) {
            ...
            foreach ($used as $num => $ds) {
                $msg[] = "Даты " . implode(', ', array_map(function ($d) { return date('d.m.y', strtotime($d)); }, $ds)) . " уже относятся к реестру №$num";
            }
    ...
    <head>
        <meta charset="UTF-8">
        <title><?= $isPreviewPrint ? 'Печать документов' : ('Реестр №' . (int)$registry_number) ?></title>
        <style>
    ...
    <body>
        ...
        <div class="period">
            Р·Р° РїРµСЂРёРѕРґ <?=date('d.m.y', strtotime($from))?> вЂ” <?=date('d.m.y', strtotime($to))?>
            <?php if ($isPreviewPrint): ?>
                <br><em>Печать без фиксации (режим без прав кладовщика)</em>
            <?php else: ?>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // UPDATE COMMENTS (AJAX) — только кладовщик
    ...
    if ($action === 'search') {
        require_csrf();
        // РѕР±СЂР°Р±РѕС‚РєР° РЅРёР¶Рµ (flash)
    } else {
        if (!$canManageRegistry) { http_response_code(403); die('Forbidden'); }
        require_csrf();
    }
...
if (!$isAuthenticated) {
    ...
    <head>
        ...
        <title>Вход | Реестр передачи документов</title>
    ...
    <main class="login-wrap">
        <section class="login-card">
            ...
            <form method="post" class="login-form">
                ...
                <input type="password" name="credential" placeholder="Пароль" required autofocus>
...
if (isset($_GET['date'])) {
    ...
    if (isValidYmdDate($requestedDate)) {
        ...
        set_alert('Неверная дата. Открыт текущий день.');
    }
}
...
<header>
    <div class="title-group">
        ...
        <button type="button" class="logout-btn" onclick="location.href='?logout=1'">Выйти</button>
    </div>
...
<div class="container">
    ...
    <?php if (!empty($dayInfo)): ?>
        <p>- Занесён в реестр №<?= (int)$dayInfo['registry_number']; ?> от <?= htmlspecialchars((string)$dayInfo['print_date']); ?>
            <?php if ((int)$dayInfo['frozen'] === 1): ?><strong class="frozen-badge">(Р·Р°РјРѕСЂРѕР¶РµРЅ)</strong><?php endif; ?>
        </p>
    <?php endif; ?>
                    ...
                    <table class="registry-table">
                        <thead>
                            <tr>
                                ...
                                <?php if ($canManageRegistry): ?><th class="actions-col no-print">Действия</th><?php endif; ?>
                            </tr>
                        </thead>
                        <tbody>
        ...
        <table>
            <thead>
                <tr>
                    <th>Имя</th>
                    <th>ИНН</th>
                    <?php if ($canManageRegistry): ?><th>Действия</th><?php endif; ?>
                </tr>
            </thead>
            <tbody>
            <?php foreach ($contractors as $c): ?>
                ...
                <?php if (!empty($searchResults)): ?>
                    <h4 style="margin-top:0;">
                        ...
                        <?php if ($searchFromDisplay || $searchToDisplay): ?>
                            ...
                            <?php if ($searchToDisplay): ?> РїРѕ <?= $searchToDisplay; ?><?php endif; ?>
                        <?php endif; ?>
                        (РЅР°Р№РґРµРЅРѕ: <?= count($searchResults); ?>)
                    </h4>
                    <table>
                        <thead>
                            <tr>
                                ...
                                <td>
                                    <?php if (!empty($res['registry_number']) && (int)$res['registry_number'] > 0): ?>
                                        ...
                                        <?php if (!empty($res['frozen'])): ?>
                                            <strong class="frozen-badge"> (Р·Р°РјРѕСЂРѕР¶РµРЅ)</strong>
                                        <?php endif; ?>
                                    <?php else: ?>
                                        <em>Не в реестре</em>
                                    <?php endif; ?>
